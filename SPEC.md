...
- **Online-Ansicht für Kunden:** Öffentliche Route (`app/(public)/studie/[id]/page.tsx`) gegated durch signierten HMAC-Token im Query-Param `?t=<token>` — kein Login nötig, aber Token + Study-ID müssen serverseitig matchen. Token-Format: `<base64url(JSON-payload)>.<base64url(HMAC-SHA256-signature)>`; Payload `{ studyId, exp, organizationId }`. Signiert via `STUDY_SHARE_HMAC_SECRET` (neue env var, mind. 32 Byte Entropie); verifiziert via constant-time `crypto.timingSafeEqual`. Default-TTL pro Link: **30 Tage** (über die Server-Action `createShareLinkAction(input)` per `expiresInDays` zwischen 1 und 365 anpassbar). Bei Token-Erzeugung wird ein `AuditLog`-Eintrag `SHARE_LINK_CREATED` geschrieben (entityType=`Study`, changeSet `{ tokenExpiresAt, ttlDays }`) — DSGVO-Nachverfolgung, **kein Token-Leak im Audit-Log**. Public-Route-Layout hat `metadata.robots = { index: false, follow: false }` (defense-in-depth gegen Suchmaschinen-Indexierung), keine Topbar, keine Session-Auth, „Sie"-Microcopy (SPEC §8.3). Bei expired-Token: eigene Error-Page mit „Sie"-Hinweis; bei invalid/malformed/Token-Studie-Mismatch: `notFound()` (kein Info-Leak). Revocation für MVP: Secret-Rotation invalidiert ALLE ausgestellten Tokens (Revocation-by-Rotation; eine separate Token-Tabelle wäre Folge-Iteration). Vorab-Light für das Phase-3-Kunden-Portal (SPEC §2.3) — kein User-Auth-Flow, kein Editing.
