...
**Affected files:** `src/lib/db.ts` (new), `src/lib/repositories/**` (16 new files: 7 repository modules + 7 repository tests + with-org + with-org test + transaction + transaction test + index barrel), `eslint.config.mjs` (no-restricted-imports rule extended with Prisma-Client group + trusted-path override block), `TASKS.md` (T-013 → ✅ Recently completed), `DECISIONS.md` (this entry).
**Open question for the user:** —
